...
export async function fetchApi<T = any>(
  ...
  const signature = generateSignature(endpoint, API_CONFIG.appId, API_CONFIG.appKey, timestamp);
  ...
  const doRequest = async (customHeaders: HeadersInit = headers): Promise<Response> => {
  ...
  if (auth && jsonData?.status_code === 401 && isTokenExpired(finalCookieString) && attempt < 1) {
    attempt++;
    ...
    const refreshRes = await fetch(`${API_CONFIG.baseUrl}${API_ENDPOINTS.AUTH.REFRESH_TOKEN}`, {
      method: 'POST',
...
export async function apiGet<T = any>(endpoint: string, options: FetchApiOptions = {}): Promise<T> {
